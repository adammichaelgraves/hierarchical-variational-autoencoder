...
from transvae.loss import hier_trans_vae_loss, vae_loss, trans_vae_loss, aae_loss, wae_loss
...
class VAEShell():
    ...
    def train(self, train_mols, val_mols, train_props=None, val_props=None,
        ...
        for epoch in range(epochs):
            ...
            # ------- initialise per‑decoder running sums & sample counters --------------
            if self.params.get('HIERARCHICAL', False):
                n_dec         = self.params['d_latent']          # N independent decoders
                train_dec_bce = torch.zeros(n_dec, device=self.device)
                val_dec_bce   = torch.zeros_like(train_dec_bce)
                train_steps   = 0                                
                val_steps     = 0
            for j, data in enumerate(train_iter):
                ...
                for i in range(self.params['BATCH_CHUNKS']):
                    ...
                    # ---------------------------------------------------------------------------
                    if self.model_type == 'transformer' and self.params.get('HIERARCHICAL', False):
                        x_list, mu, logvar, pred_len, pred_prop = \
                            self.model(src, tgt, true_prop, src_mask, tgt_mask)
                        true_len = src_mask.sum(dim=-1)

                        loss, tot_recon, mask_loss, per_dec_recon, kld, prop_bce = \
                            hier_trans_vae_loss(src, x_list, mu, logvar, true_len, pred_len,
                                                true_prop, pred_prop, self.params['CHAR_WEIGHTS'],
                                                self, beta)

                        train_dec_bce += torch.stack(per_dec_recon).detach()
                        train_steps   += 1
                        avg_bcemask_losses.append(mask_loss.item())
                        avg_kld_losses.append(kld.item())
                        avg_prop_bce_losses.append(prop_bce.item())

                        bce = tot_recon                 # so unconditional append works
                        if j % 50 == 0 and i == 0:
                            per_str = ", ".join([f"d{d+1}:{l.item():.3f}"
                                                 for d, l in enumerate(per_dec_recon)])
                            print(f"[epoch {epoch:03d} | batch {j:03d}] "
                                  f"tot:{loss.item():.3f} | mask:{mask_loss.item():.3f} | "
                                  f"kld:{kld.item():.3f} | {per_str}")
                    else:
                        # This is the vanilla VAE block - just one decoder
                        x_out, mu, logvar, pred_len, pred_prop = \
                            self.model(src, tgt, true_prop, src_mask, tgt_mask)
                        true_len = src_mask.sum(dim=-1)
                        loss, bce, bce_mask, kld, prop_bce = \
                            trans_vae_loss(src, x_out, mu, logvar, true_len, pred_len,
                                           true_prop, pred_prop, self.params['CHAR_WEIGHTS'],
                                           self, beta)
            ...
            for j, data in enumerate(val_iter):
                ...
                for i in range(self.params['BATCH_CHUNKS']):
                    ...
                    if self.model_type == 'transformer' and self.params.get('HIERARCHICAL', False):
                        ...
                        val_dec_bce   += torch.stack(per_dec_recon).detach()
                        val_steps     += 1
                        ...
                        #Vanilla VAE block - single decoder
                        x_out, mu, logvar, pred_len, pred_prop = \
                            self.model(src, tgt, true_prop, src_mask, tgt_mask)
                        true_len = src_mask.sum(dim=-1)
                        loss, bce, bce_mask, kld, prop_bce = \
            ...
            epoch_time = round(epoch_end_time - epoch_start_time, 5)

            if self.params.get('HIERARCHICAL', False):
                if train_steps > 0:
                    mean = (train_dec_bce / train_steps).tolist()
                    print("train d‑wise BCE: " + " ".join(f"{x:.3f}" for x in mean), flush=True)
                if val_steps > 0:
                    mean = (val_dec_bce / val_steps).tolist()
                    print("val   d‑wise BCE: " + " ".join(f"{x:.3f}" for x in mean), flush=True)
    ...
    def greedy_decode(self, mem, src_mask=None):
        ...
        for i in range(max_len):
            if i%120==0: print("decoding sequences of max length ",max_len,"current position: ",i)
    ...
    def reconstruct(self, data, method='greedy', log=True, return_mems=True, return_str=True):
        ...
        with torch.no_grad():
            ...
            for j, data in enumerate(data_iter):
                print('reconstructing batch: ', j, " out of : ",len(data_iter))
                if log:
